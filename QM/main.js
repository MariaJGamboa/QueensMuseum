...
let currentDifficulty = "children"; // default
...
// ==============================
// AGGIORNAMENTO CONTENUTI EXHIBITION
// ==============================
function updateContent() {
    ...
    // testi
    const textData = item.text.historical[currentDifficulty];
...
setInterval(nextImage, 4000); // carousel automatico

// ==============================
// NAVIGAZIONE TRA ITEMS
// ==============================
...
function buildList() {
    ...
    // click sulle stanze SVG → primo item della stanza
        ...
        if (roomEl) {
            ...
            roomEl.addEventListener('click', () => {
                const firstItem = items
                    .filter(i => i.metadata.room === roomMap[roomClass])
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // se siamo su exhibition.html
    if (window.location.pathname.includes("exhibition.html")) {
        updateContent();
    }

    // se siamo su Map.html
    if (window.location.pathname.includes("Map.html")) {
        buildList();
    }

    // gestione bottoni narrativa
    const histBtn = document.getElementById("hist");
    const popBtn = document.getElementById("pop-cult");
    if (histBtn) histBtn.addEventListener('click', () => { currentNarrative = "historical"; buildList(); });
    if (popBtn) popBtn.addEventListener('click', () => { currentNarrative = "popculture"; buildList(); });
});
